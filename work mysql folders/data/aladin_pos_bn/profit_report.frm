TYPE=VIEW
query=select `tsp`.`sell_id` AS `sell_id`,`ts`.`seller` AS `seller`,`tc`.`customer_id` AS `customer_id`,`tc`.`customer_name` AS `customer_name`,`tsp`.`product_id` AS `product_id`,sum(`tsp`.`quantity`) AS `quantity`,`tsp`.`purchase_price` AS `purchase_price`,`tsp`.`unit_price` AS `unit_price`,`ts`.`date` AS `date` from ((`aladin_pos_bn`.`tbl_sell_products` `tsp` join `aladin_pos_bn`.`tbl_customer` `tc` on((`tc`.`customer_id` = `tsp`.`customer_id`))) join `aladin_pos_bn`.`tbl_sell` `ts` on((`tsp`.`sell_id` = `ts`.`sell_id`))) where (`tsp`.`status` = \'1\') group by `tsp`.`sell_id`,`tsp`.`product_id`
md5=6f79868b8300bd1f8cd70a7ff2bf40a1
updatable=0
algorithm=0
definer_user=root
